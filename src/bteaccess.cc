...
#define TERMINAL_FROM_ACCESSIBLE(a) (BTE_TERMINAL(ctk_accessible_get_widget(CTK_ACCESSIBLE(a))))

#define IMPL(t) (_bte_terminal_get_impl(t))
#define IMPL_FROM_WIDGET(w) (IMPL(BTE_TERMINAL(w)))
#define IMPL_FROM_ACCESSIBLE(a) (IMPL_FROM_WIDGET(ctk_accessible_get_widget(CTK_ACCESSIBLE(a))))
...
static gunichar bte_terminal_accessible_get_character_at_offset(AtkText *text,
								gint offset);

static const char *bte_terminal_accessible_action_names[] = {
        "menu",
        NULL
};

static const char *bte_terminal_accessible_action_descriptions[] = {
        "Popup context menu",
        NULL
};

static void bte_terminal_accessible_text_iface_init(AtkTextIface *iface);
...
{
	_bte_debug_print(BTE_DEBUG_SIGNALS|BTE_DEBUG_ALLY,
                                                      ...
                                                      GArray **old_characters)
{
	...
	if (priv->snapshot_contents_invalid) {
		...
		for (i = 0, row = 0; i < priv->snapshot_characters->len; i++) {
			...
			if ((i == 0) || (attrs.row != row)) {
				_bte_debug_print(BTE_DEBUG_ALLY,
	...
	bte_terminal_get_cursor_position(terminal, &ccol, &crow);
	_bte_debug_print(BTE_DEBUG_ALLY,
			"Cursor at (%ld, " "%ld).\n", ccol, crow);
...
bte_terminal_accessible_text_modified(BteTerminal *terminal, gpointer data)
{
	...
	if ((offset < olen) || (offset < clen)) {
        ...
        bte_terminal_accessible_maybe_emit_text_caret_moved(accessible);
				      ...
				      gpointer data)
{
	...
	guint i, len;
        ...
        row_count = bte_terminal_get_row_count(terminal);
					  ...
					  gpointer data)
{
	...
	while (visible) {
		if (ctk_widget_get_toplevel(widget) == widget) {
			break;
		}
		if (widget == NULL) {
			break;
		}
		visible = visible && (ctk_widget_get_visible(widget));
...
{
	BteTerminal *terminal = BTE_TERMINAL (data);
	...
	g_signal_connect(terminal, "text-inserted",
			 G_CALLBACK(bte_terminal_accessible_text_modified),
			 obj);
	...
	g_signal_connect(terminal, "text-scrolled",
			 G_CALLBACK(bte_terminal_accessible_text_scrolled),
			 obj);
	g_signal_connect(terminal, "cursor-moved",
			 G_CALLBACK(bte_terminal_accessible_invalidate_cursor),
			 obj);
	g_signal_connect(terminal, "window-title-changed",
			 G_CALLBACK(bte_terminal_accessible_title_changed),
			 obj);

	g_signal_connect(terminal, "visibility-notify-event",
		G_CALLBACK(bte_terminal_accessible_visibility_notify), obj);
	g_signal_connect(terminal, "selection-changed",
		G_CALLBACK(bte_terminal_accessible_selection_changed), obj);

	atk_object_set_name(obj, "Terminal");
        window_title = bte_terminal_get_window_title(terminal);
...
_bte_terminal_accessible_init (BteTerminalAccessible *accessible)
{
	...
	_bte_debug_print(BTE_DEBUG_ALLY, "Initialising accessible peer.\n");
...
{
        BteTerminalAccessible *accessible = BTE_TERMINAL_ACCESSIBLE(object);
	BteTerminalAccessiblePrivate *priv = (BteTerminalAccessiblePrivate *)_bte_terminal_accessible_get_instance_private(accessible);
        CtkWidget *widget;
	gint i;

	_bte_debug_print(BTE_DEBUG_ALLY, "Finalizing accessible peer.\n");
	...
	if (widget != NULL) {
		g_signal_handlers_disconnect_matched(widget,
						     ...
						     0, 0, NULL,
						     (void *)bte_terminal_accessible_text_modified,
						     object);
		g_signal_handlers_disconnect_matched(widget,
						     (GSignalMatchType)(G_SIGNAL_MATCH_FUNC | G_SIGNAL_MATCH_DATA),
						     0, 0, NULL,
						     (void *)bte_terminal_accessible_text_scrolled,
						     object);
		g_signal_handlers_disconnect_matched(widget,
						     (GSignalMatchType)(G_SIGNAL_MATCH_FUNC | G_SIGNAL_MATCH_DATA),
						     0, 0, NULL,
						     (void *)bte_terminal_accessible_invalidate_cursor,
						     object);
		g_signal_handlers_disconnect_matched(widget,
						     (GSignalMatchType)(G_SIGNAL_MATCH_FUNC | G_SIGNAL_MATCH_DATA),
						     0, 0, NULL,
						     (void *)bte_terminal_accessible_title_changed,
						     object);
		g_signal_handlers_disconnect_matched(widget,
						     (GSignalMatchType)(G_SIGNAL_MATCH_FUNC | G_SIGNAL_MATCH_DATA),
						     0, 0, NULL,
						     (void *)bte_terminal_accessible_visibility_notify,
						     object);
	}
	...
	G_OBJECT_CLASS(_bte_terminal_accessible_parent_class)->finalize(object);
}
...
{
        BteTerminalAccessible *accessible = BTE_TERMINAL_ACCESSIBLE(text);
	BteTerminalAccessiblePrivate *priv = (BteTerminalAccessiblePrivate *)_bte_terminal_accessible_get_instance_private(accessible);
	int start, end;
	gchar *ret;

	g_assert(BTE_IS_TERMINAL_ACCESSIBLE(accessible));
...
static gchar *
bte_terminal_accessible_get_text_somewhere(AtkText *text,
					   gint offset,
					   ...
					   gint *end_offset)
{
	...
	switch (boundary_type) {
		...
		case ATK_TEXT_BOUNDARY_WORD_START:
			...
			while (offset > 0) {
				prev = bte_terminal_accessible_get_character_at_offset(text, offset - 1);
			...
			if (direction == direction_next) {
				while (offset < (int) priv->snapshot_characters->len) {
					next = bte_terminal_accessible_get_character_at_offset(text, offset);
			...
			while (offset < (int) priv->snapshot_characters->len) {
				current = bte_terminal_accessible_get_character_at_offset(text, offset);
	...
	*end_offset = end = CLAMP(end, start, priv->snapshot_characters->len);
	return bte_terminal_accessible_get_text(text, start, end);
}

static gchar *
bte_terminal_accessible_get_text_before_offset(AtkText *text, gint offset,
					       AtkTextBoundary boundary_type,
					       gint *start_offset,
					       gint *end_offset)
{
	...
	bte_terminal_accessible_update_private_data_if_needed(accessible,
							      NULL, NULL);
	return bte_terminal_accessible_get_text_somewhere(text,
							  offset,
							  boundary_type,
							  direction_previous,
							  start_offset,
...
static gchar *
bte_terminal_accessible_get_text_after_offset(AtkText *text, gint offset,
					      AtkTextBoundary boundary_type,
					      gint *start_offset,
					      gint *end_offset)
{
...
static gchar *
bte_terminal_accessible_get_text_at_offset(AtkText *text, gint offset,
					   AtkTextBoundary boundary_type,
					   gint *start_offset,
					   gint *end_offset)
{
...
bte_terminal_accessible_get_character_at_offset(AtkText *text, gint offset)
{
	...
	unichar = bte_terminal_accessible_get_text(text, offset, offset + 1);
...
static AtkAttributeSet *
bte_terminal_accessible_get_default_attributes(AtkText *text)
{
	return NULL;
}

static void
bte_terminal_accessible_get_character_extents(AtkText *text, gint offset,
					      gint *x, gint *y,
					      ...
					      AtkCoordType coords)
{
	...
	bte_terminal_accessible_update_private_data_if_needed(accessible,
							      NULL, NULL);
	terminal = BTE_TERMINAL (ctk_accessible_get_widget (CTK_ACCESSIBLE (text)));

	atk_component_get_extents (ATK_COMPONENT (text), &base_x, &base_y, &w, &h, coords);
	xy_from_offset (priv, offset, x, y);
	cell_width = bte_terminal_get_char_width (terminal);
	cell_height = bte_terminal_get_char_height (terminal);
	*x *= cell_width;
	*y *= cell_height;
	*width = cell_width;
	*height = cell_height;
...
static gint
bte_terminal_accessible_get_offset_at_point(AtkText *text,
					    gint x, gint y,
					    AtkCoordType coords)
{
        BteTerminalAccessible *accessible = BTE_TERMINAL_ACCESSIBLE(text);
...
bte_terminal_accessible_get_n_selections(AtkText *text)
{
	...
	terminal = BTE_TERMINAL (widget);
	return (bte_terminal_get_has_selection (terminal)) ? 1 : 0;
}
				      ...
				      gint *start_offset, gint *end_offset)
{
	...
	if (widget == NULL) {
        ...
        try {
                ...
                if (impl->m_selection_resolved.empty() || impl->m_selection[BTE_SELECTION_PRIMARY] == nullptr)
                        return nullptr;
                ...
                *end_offset = offset_from_xy (priv, impl->m_selection_resolved.end_column(), impl->m_selection_resolved.end_row());
                ...
                return g_strdup(impl->m_selection[BTE_SELECTION_PRIMARY]->str);
        } catch (...) {
                return nullptr;
        }
}
					 ...
					 gint selection_number)
{
	...
	terminal = BTE_TERMINAL (widget);
        auto impl = IMPL_FROM_WIDGET(widget);
	if (selection_number == 0 && bte_terminal_get_has_selection (terminal)) {
                try {
                        impl->deselect_all();
                } catch (...) {
                }
				      ...
				      gint start_offset, gint end_offset)
{
	...
	if (bte_terminal_get_has_selection (terminal)) {
                try {
                        impl->deselect_all();
                } catch (...) {
                }
	...
	return bte_terminal_accessible_add_selection (text, start_offset, end_offset);
}
...
static void
bte_terminal_accessible_text_iface_init(AtkTextIface *text)
{
	text->get_text = bte_terminal_accessible_get_text;
	text->get_text_after_offset = bte_terminal_accessible_get_text_after_offset;
	text->get_text_at_offset = bte_terminal_accessible_get_text_at_offset;
	text->get_character_at_offset = bte_terminal_accessible_get_character_at_offset;
	text->get_text_before_offset = bte_terminal_accessible_get_text_before_offset;
	text->get_caret_offset = bte_terminal_accessible_get_caret_offset;
	text->get_run_attributes = bte_terminal_accessible_get_run_attributes;
	text->get_default_attributes = bte_terminal_accessible_get_default_attributes;
	text->get_character_extents = bte_terminal_accessible_get_character_extents;
	text->get_character_count = bte_terminal_accessible_get_character_count;
	text->get_offset_at_point = bte_terminal_accessible_get_offset_at_point;
	text->get_n_selections = bte_terminal_accessible_get_n_selections;
	text->get_selection = bte_terminal_accessible_get_selection;
	text->add_selection = bte_terminal_accessible_add_selection;
	text->remove_selection = bte_terminal_accessible_remove_selection;
	text->set_selection = bte_terminal_accessible_set_selection;
	text->set_caret_offset = bte_terminal_accessible_set_caret_offset;
}

static gboolean
bte_terminal_accessible_set_extents(AtkComponent *component,
				    gint x, gint y,
				    gint width, gint height,
...
static gboolean
bte_terminal_accessible_set_position(AtkComponent *component,
				     gint x, gint y,
				     AtkCoordType coord_type)
{
...
bte_terminal_accessible_set_size(AtkComponent *component,
				 gint width, gint height)
{
        BteTerminalAccessible *accessible = BTE_TERMINAL_ACCESSIBLE(component);
	CtkWidget *widget;
	...
	if (widget == NULL)
        ...
        try {
                ...
                bte_terminal_set_size(terminal, columns, rows);
                return (bte_terminal_get_row_count (terminal) == rows) &&
                        (bte_terminal_get_column_count (terminal) == columns);
        } catch (...) {
                return false;
        }
}
...
static AtkObject *
bte_terminal_accessible_ref_accessible_at_point(AtkComponent *component,
						gint x, gint y,
						AtkCoordType coord_type)
{
...
static void
bte_terminal_accessible_component_iface_init(AtkComponentIface *component)
{
	component->ref_accessible_at_point = bte_terminal_accessible_ref_accessible_at_point;
	component->set_extents = bte_terminal_accessible_set_extents;
	component->set_position = bte_terminal_accessible_set_position;
	component->set_size = bte_terminal_accessible_set_size;
}

/* AtkAction interface */

static gboolean
bte_terminal_accessible_do_action (AtkAction *accessible, int i)
{
	CtkWidget *widget;
...
static int
bte_terminal_accessible_get_n_actions (AtkAction *accessible)
{
	return LAST_ACTION;
}
...
{
        BteTerminalAccessible *accessible = BTE_TERMINAL_ACCESSIBLE(action);
	BteTerminalAccessiblePrivate *priv = (BteTerminalAccessiblePrivate *)_bte_terminal_accessible_get_instance_private(accessible);
        ...
        if (priv->action_descriptions[i]) {
                ...
                return bte_terminal_accessible_action_descriptions[i];
        }
}

static const char *
bte_terminal_accessible_action_get_name (AtkAction *accessible, int i)
{
        g_return_val_if_fail (i < LAST_ACTION, NULL);

        return bte_terminal_accessible_action_names[i];
}

static const char *
bte_terminal_accessible_action_get_keybinding (AtkAction *accessible, int i)
{
        g_return_val_if_fail (i < LAST_ACTION, NULL);
...
static gboolean
bte_terminal_accessible_action_set_description (AtkAction *action,
                                                int i,
...
static void
bte_terminal_accessible_action_iface_init(AtkActionIface *action)
{
	action->do_action = bte_terminal_accessible_do_action;
	action->get_n_actions = bte_terminal_accessible_get_n_actions;
	action->get_description = bte_terminal_accessible_action_get_description;
	action->get_name = bte_terminal_accessible_action_get_name;
	action->get_keybinding = bte_terminal_accessible_action_get_keybinding;
	action->set_description = bte_terminal_accessible_action_set_description;
}
...
_bte_terminal_accessible_class_init(BteTerminalAccessibleClass *klass)
{
	...
	gobject_class->finalize = bte_terminal_accessible_finalize;

	atk_object_class->initialize = bte_terminal_accessible_initialize;
}
